...
status: built 2026-09-19 (phases 0-6d, commits 9746add..2e5ffc3) — not yet verified on a device (V12 unrun)
sources:
...
- **D-W12. No GPS, no NFC, no new native dependency, no Google ML Kit.** Two justified pure-JS dependencies (`jsqr`, `jpeg-js`). One `app.json` change: expo-camera `barcodeScannerEnabled: false` — the intent is to drop the ML Kit dependency, but **verified on the 0.2.0 APK it does not remove it** (see D-W15). *Why:* consumer GNSS is 4.9 m under open sky and 2–10 m under canopy against 3–5 m tree spacing; NFC reads at 0–3 cm and forces minSdk 31; everything else needed is installed; removing ML Kit strengthens D-17.
...
- **D-W15. No Google ML Kit, anywhere.** Its Terms (2025-05-14) state the APIs "may contact Google servers from time to time" and "send metrics about the performance and utilization of the APIs in your app to Google" — a third network destination that the airplane-mode test cannot rule out (metrics can flush later). Codes are decoded by `jsqr` on `jpeg-js` pixels from a **deliberately captured, 800 px** photo ("Scan tag" button); import-side decoding runs on the 800 px copy. QR only in v1.
  ...
  **Measured on the 0.2.0 APK (2026-09-19), stated precisely.** What is enforced is that the app **never invokes** an ML Kit API: `grep` over `src` and `App.tsx` finds zero uses of `barcodeScannerSettings`, `onBarcodeScanned`, `scanFromURLAsync`, `launchScanner` or `onModernBarcodeScanned`, and the Android analyzer is bound only behind `shouldScanBarcodes` (`ExpoCameraView.kt:540`), which only that JS prop can set. What is **not** true is that the library is absent: the APK still contains `assets/mlkit_barcode_models/*.tflite` and the barcode-scanning artifacts, because Expo SDK 57 consumes expo-camera as a **precompiled Maven artifact** (`host.exp.exponent:expo.modules.camera:57.0.1`) whose POM declares `com.google.mlkit:barcode-scanning` and `play-services-code-scanner` as `implementation`; the gradle property the plugin writes (`expo.camera.barcode-scanner-enabled=false`, confirmed present in the generated `gradle.properties`) only gates the *source* build, and an `expo.inlineModules.watchedDirectories` override did not change the resolved release classpath either. This is the state every previous release shipped in, not a regression. Unused library code makes no calls, so the two-network-destination promise stands. Follow-up (Phase 7): either build expo-camera from source so the flag applies, or drop the claim and verify by runtime traffic instead.
